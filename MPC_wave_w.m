...
% in which the controller has access to an imperfect plant model with an additive disturbance w.  
...
Bw = zeros(2*N,1); % the disturbance is a constant force applied to the first node.
Bw(end) = 1;
w =@(t) Bw;
...
%% compute the infinite horizon optimal control and the MPC limit by solving an ARE
...
[Zopt, duration] = compute_Xw(A-B*(R\(B.'*Pinf)), X0, B, 0*U, tgrid, Mass, w);
Vopt = -(R\(B.'*Pinf))*Zopt;
Vopt = (Vopt(1:end-1)+Vopt(2:end))/2;

xiw =@(t) w(t) + (B*(R\B')) * (Acl.'\(Pinf*Bw));
[Xopt, duration] = compute_Xw(A-B*(R\(B.'*Pinf)), X0, B, 0*U, tgrid, Mass, xiw );
Uopt = -(R\(B.'*Pinf))*Xopt + (R\B')*(Acl.'\(Pinf*Bw));
...
for kk = 1:nsteps    
    ...
    % find the optimal control in the current time interval (without considering w)
    ...
    % k\tau + T] (considering w)
    [Xoptkk,  duration] = compute_Xw(A, X0kk, B, Uoptkk(1:(ind3-ind1)), tgrid(ind1:ind3), Mass, w);
    [Xoptkk2, duration] = compute_Xw(A, X0kk, B, Uoptkk, tgrid(ind1:ind2), Mass, w);
...
ylabel 'u(t)'
xlabel 't'
name = ['MPCw_T=', num2str(T*10000), '_tau=', num2str(tau*10000), '.jpeg'];
...
ylabel '|y(t)|'
xlabel 't'
name = ['MPCwX_T=', num2str(T*10000), '_tau=', num2str(tau*10000), '.jpeg'];
print('-djpeg','-r500', name)
% saveas(fig, ['MPC_T=', num2str(T*10000), '_tau=', num2str(tau*10000), '.fig']);