...
Bw = zeros(N,1); % the disturbance is applied to the first node and is constant
Bw(end) = 1;
w =@(t) Bw;

%% define the considered time grid
...
[Xopt, duration] = compute_Xw(A-B*(R\(B.'*Pinf)), X0, B, 0*U, tgrid, Mass, w);
Uopt = -(R\(B.'*Pinf))*Xopt;
...
plot(tau_list1, 0.11 + 0.29*tau_list1, 'Color', 0.6*[1 1 1], 'linewidth', 2)
xlabel '\tau'
