...
[Xopt, duration] = compute_X(Acl, X0, B, 0*U, tgrid, Mass);
Uopt = -(R\(B.'*Pinf))*Xopt;
...
for kk = 1:nsteps    
    ...
    [Xoptkk,  duration] = compute_X(A, X0kk, B, Uoptkk(1:ind4), tgrid(ind1:ind3), Mass);
...
ylim([-10 12])
grid on
legend([hU, hUMPC, hUp], {'Infinite-horizon control u_\infty^*', 'RHC control u_{RHC}', 'RHC controls on [k\tau, k\tau+T]'})
ylabel 'u(t)'
xlabel 't'
name = ['MPC_T=', num2str(T*10000), '_tau=', num2str(tau*10000), '.jpeg'];
...
ylim([0 5])
grid on
legend([hX, hXMPC, hXp], {'Infinite-horizon trajectory x_\infty^*', 'RHC trajectory x_{RHC}', 'RHC trajectories on [k\tau, k\tau+T]'})
ylabel '|x(t)|'
xlabel 't'
name = ['MPCX_T=', num2str(T*10000), '_tau=', num2str(tau*10000), '.jpeg'];
print('-djpeg','-r500', name)
% saveas(fig, ['MPC_T=', num2str(T*10000), '_tau=', num2str(tau*10000), '.fig']);